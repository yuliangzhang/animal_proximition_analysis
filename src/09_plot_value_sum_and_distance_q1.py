...
OUT_DIR = "data/dating_analysis/proximition_distance_q1_plot"
...
def load_data(path: str) -> pd.DataFrame:
    df = pd.read_csv(path)
    req = {"ram_eid", "ewe_eid", "time_split", "value_sum", "distance_q1", "distance_cnt"}
    ...
    df["distance_cnt"] = pd.to_numeric(df.get("distance_cnt", 0), errors="coerce").fillna(0).astype(int)
    # Convert distance_q1: 'INF' -> NaN; numeric otherwise
    df["distance_q1_num"] = pd.to_numeric(df["distance_q1"], errors="coerce")
    # Valid distance points: finite, <=150m, and distance_cnt >= 20
    mask_valid_dist = (
        np.isfinite(df["distance_q1_num"]) &
        (df["distance_q1_num"] <= 150.0) &
        (df["distance_cnt"] >= 20)
    )
    df["distance_q1_valid"] = mask_valid_dist
    return df
...
def plot_pair(ram: str, ewe: str, sub: pd.DataFrame) -> str:
    ...
    # For distance q1, only plot valid points
    y_dist = sub["distance_q1_num"].to_numpy()
    valid = sub["distance_q1_valid"].to_numpy()
    ...
    # Plot only valid distance_q1 points
    ax2.plot(
        x[valid],
        y_dist[valid],
        color="#2ca02c",
    ...
    fname = f"trend_q1_{sanitize(ram)}_{sanitize(ewe)}.png"
