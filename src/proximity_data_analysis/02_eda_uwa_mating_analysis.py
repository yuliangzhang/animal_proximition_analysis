...
1) Data quality screening and low-signal pair detection.
2) Plot-day management anomaly mining using robust statistics.
3) Pair-level mating inference with biologically constrained filtering.
4) Year + paddock result export (6 files in total).
5) EDA tables, figures, and bilingual Markdown reports.
...
### 3.3 对你提到的 2023-03-02 High Shade 的验证
- `sync_prop = {hs_0302_sync:.3f}`，`rz_sync = {hs_0302_rz:.3f}`，达到异常阈值。
- 因此该日期被识别为群体同步异常日，在交配判定中**排除**，避免“同一天同一公羊对应大量母羊”误判。
- 对比 2023-02-15（同 paddock）`sync_prop = {hs_0215_sync:.3f}`，同样属于实验早期高同步日。
...
### 3.2 Management anomaly day mining
...
### 3.3 Validation of the user-observed day (2023-03-02, High Shade)
- `sync_prop = {hs_0302_sync:.3f}`, `rz_sync = {hs_0302_rz:.3f}`: this day is synchronized and flagged.
- Therefore, it is excluded from mating inference to avoid many-to-many false positives.
...
def run_pipeline() -> None:
    ...
    bundles = {year: load_year_data(year) for year in YEARS}

    anomaly_parts = []
    pair_parts = []
    for year in YEARS:
        bundle = bundles[year]
        anomaly_df = detect_management_anomalies(bundle)
        pair_df = infer_pair_candidates(bundle, anomaly_df)
    ...
    tables = build_summary_tables(final_df, pair_all, anomaly_all)
    save_tables(final_df, pair_all, anomaly_all, tables, split_index)

    plot_day_heatmaps(anomaly_all)
    plot_management_focus(anomaly_all)
    plot_mating_overview(final_df, tables)

    write_reports(final_df, pair_all, anomaly_all, tables, split_index)
